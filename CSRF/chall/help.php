...
<header class="cd-header flex flex-column flex-center">
  <div class="text-component text-center">
    <h1>FAQ Template</h1>
    <p>👈 <a class="cd-article-link" href="dashboard.php">Return to Dashboard</a></p>
  </div>
</header>
	...
	<div class="cd-faq__items">
		...
		<ul id="tech" class="cd-faq__group">
			...
			<li class="cd-faq__item">
				...
				<div class="cd-faq__content">
          <div class="text-component">
            <p>My problem is not stated above, therefore I want to personally reach the support team and find the solution.</p>
          </div>
				</div> <!-- cd-faq__content -->
			</li>

			<li class="cd-faq__item">
				<a class="cd-faq__trigger" href="#0"><span>Contact to customer care</span></a>
				<div class="cd-faq__content">
          <div class="text-component">
            <p>You can reach our team by calling us at toll free number   +91 8800 8800 88</p>
          </div>
				</div> <!-- cd-faq__content -->
			</li>

			<li class="cd-faq__item">
				<a class="cd-faq__trigger" href="#0"><span>Contact via email</span></a>
				<div class="cd-faq__content">
          <div class="text-component">
            <p>You can reach us via email support-desk@vulnfreak.org. Our support team will reach you out in 24 hrs.</p>
          </div>
				</div> <!-- cd-faq__content -->
			</li>

			<li class="cd-faq__item">
				<a class="cd-faq__trigger" href="#0"><span>Chat with our support team</span></a>
				<div class="cd-faq__content">
          <div class="text-component">
        ...
        <?php
            ...
            if (isset($_POST['submit']) and $_POST['chat'] != null) {
                $string = $_POST['chat'];
                $matches = [];
                $pattern = '/^(.*?)\/check.php\?check=[a-z A-z 0-9 \\ \!\@\#\$\%\*\(\)\_\+\-\=\,\<\>\?\/\'\:\;\|\{\[\}\]]{0,}&/';
                if(preg_match($pattern, $string, $matches))
                {
                  $begin=strpos("$matches[0]","=");
                  $begin+=1;
                  $end=strpos("$matches[0]","&");
                  $strlen=strlen($matches[0]);
                  $end= ($strlen-$begin)-1;
                  $sub = substr("$matches[0]", $begin,$end);
				  if($sub == $_SESSION['uname'])
                  {
					 	include "connection.php";
						
						$username=mysqli_real_escape_string($conn,htmlspecialchars($sub));
						...
						if(mysqli_num_rows($queryexec) == 1)
						{
							...
							$queryexec=mysqli_query($conn,$query);
							$sql = "SELECT * FROM loginportal WHERE username=?";
							...
							$email = $user['email'];	
							system("curl https://member.vulnfreak.org/mailing.php?email=". $email);

							
						?>

							<p style="text-align:justify"> For further communication our support team will contact you via email provided by you.</p>
